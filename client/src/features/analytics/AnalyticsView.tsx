...
function PredictionsTab({
  ...
  const summaryTone =
    viewModel.predictions.modelStatus === 'gemini_enriched'
      ? 'Forecast na may tulong ng AI'
      : isLoading
        ? 'Ina-update ang lokal na forecast'
        : 'Lokal na forecast';
    ...
    <View style={styles.tabStack}>
      <ForecastHero
        body={
          leadItem
            ? leadItem.detail
            : viewModel.predictions.emptyState ?? 'Lalabas ang forecast kapag may sapat nang tala ng benta.'
        }
        summaryTone={summaryTone}
        title={leadItem ? leadItem.itemName : 'Wala pang sapat na forecast'}
        ...
        <ShoppingList
          emptyText={
            viewModel.predictions.emptyState
              ? viewModel.predictions.emptyState
              : `Mukhang sapat pa ang stock mo para sa susunod na ${activePreset?.days ?? 7} araw.`
          }
          items={shoppingItems}
        />
      </CardSurface>
...
function ForecastHero({
  ...
  return (
    <View style={styles.forecastHero}>
      ...
      <Text style={styles.forecastLabel}>Taya sa demand</Text>
...
function SellingList({ items }: { items: AnalyticsListItem[] }) {
  if (items.length === 0) {
    return <Text style={styles.emptyText}>Wala pang naitalang galaw dito.</Text>;
  }
...
function TrendDetectionList({
  ...
  return (
    <View style={styles.trendList}>
      <Text style={styles.trendLead}>Mga item na bumibilis o bumabagal ang galaw</Text>
      {points.map((point) => (
      ...
      {items.map((item) => (
        <View key={`${item.itemId}-${item.itemName}`} style={styles.predictionRow}>
          ...
          <Pressable accessibilityRole="button" onPress={noop} style={styles.restockButton}>
            <Text style={styles.restockButtonText}>Magmungkahi ng dagdag-stock</Text>
          </Pressable>
        </View>
      ))}
    </View>
      ...
      {items.map((item) => (
        <View key={`${item.itemId}-${item.horizonDays}`} style={styles.shoppingRow}>
          ...
          <View style={styles.shoppingMain}>
            <Text style={styles.rowTitle}>{item.itemName}</Text>
            <Text style={styles.shoppingQuantity}>{`Bumili ng ${item.recommendedBuyQuantity} ${item.unit}`}</Text>
...
function RecommendationList({
  ...
  if (recommendations.length === 0) {
    return <Text style={styles.emptyText}>Wala pang buod ng mungkahi.</Text>;
  }
    ...
    <View style={styles.recommendationStack}>
      {recommendations.map((recommendation, index) => (
        <View key={`${recommendation.title}-${recommendation.body}`} style={styles.recommendationCard}>
          {index === 0 ? <Text style={styles.recommendationLead}>Pangunahing payo</Text> : null}
